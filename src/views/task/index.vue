    ...
    <div class="layout_main_top">
      <div class="layout_main_top_left">
        <div class="layout_task" v-for="(item, index) of topArr" :key="index">
          <div
            @click="task(item.status)"
            :class="{ active: changecolor == item.status }"
          >
            <h3>{{ item.num }}</h3>
      ...
      <div style="padding-top: 25px; padding-left: 0px; font-size: 12px">
        未完成包括：未开始、进行中、超时、暂停中的任务
      </div>
    </div>
      ...
      <el-tab-pane label="任务列表" name="second" class="tab-task">
        <div ref="drawer-parent">
            ...
            <el-col style="text-align: right">
              <div style="display: flex; justify-content: flex-end">
                ...
                <el-button
                  ...
                  >查询</el-button
                >
                <el-button @click="reTask(changecolor)" type="primary"
                  >重置</el-button
                >
              </div>
            </el-col>
          ...
          >
            ...
            <el-table-column label="项目" header-align="left">
              <template slot-scope="scope">
                <router-link
                  ...
                  :to="{
                    name: 'project-detail',
                    params: { id: scope.row.project.id },
                    query: { type: scope.row.project.pro_type },
                  }"
            ...
            >
              <template slot-scope="scope">
                <el-image
                  ...
                  :preview-src-list="[
                    $store.state.BASE_URL + scope.row.asset.image,
                  ]"
                >
                  ...
                  <div slot="error" class="image-slot">
                    <el-image
                      :src="
                        ...
                        'images/appfile/1573029716.780075picture.png'
                      "
                    ></el-image>
                  </div>
                </el-image>
            ...
            >
              <template slot-scope="scope">
                <div @click="show(scope.row.asset.id)">
            ...
            <el-table-column label="截止日期" header-align="left" width="100px">
              <template slot-scope="scope">{{
                scope.row.task.end_date | dateFormat
              }}</template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <div
                  v-if="
                    scope.row.task.status != 3 &&
                    scope.row.task.status != 4 &&
                    scope.row.task.status != 6
                  "
                >
                  <el-select
                ...
                <el-progress
                  :stroke-width="12"
                  :percentage="
                    scope.row.task.schedule ? scope.row.task.schedule : 0
                  "
                  v-if="
                    scope.row.task.status != 3 && scope.row.task.status != 4
                  "
                ></el-progress>
                <div v-if="scope.row.task.status == 3">
                  {{ scope.row.task.statements }}
                </div>
              </template>
            </el-table-column>
            <el-table-column
              label="提交次数"
              prop="task.submit_num"
            ></el-table-column>
            <el-table-column
              label="难度"
              header-align="left"
            ...
            >
              <template slot-scope="scope">{{
                scope.row.task.grade | taskgrade
              }}</template>
            </el-table-column>
            <el-table-column
              label="优先级"
            ...
            <el-table-column label="开始日期" header-align="left" width="100px">
              <template slot-scope="scope">{{
                scope.row.task.start_date | dateFormat
              }}</template>
            </el-table-column>
            <el-table-column
              prop="total_hour"
            ...
            >
              <template slot-scope="scope">{{
                scope.row.task.latest_submit_time | dateFormat
              }}</template>
            </el-table-column>
            <el-table-column
              prop="task.id"
            ...
            >
              <template slot-scope="scope">
                <div @click="taskBoardRightShow(scope.row)">
                  {{ scope.row.task.id }}
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="缩略图" align="center">
              <template slot-scope="scope">
                <el-image
                  ...
                  :preview-src-list="[
                    $store.state.BASE_URL + scope.row.task.asset_image,
                  ]"
                >
                  <div slot="placeholder" class="image-slot">
                    加载中
